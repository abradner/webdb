...
module CarrierWave
  module Storage
    ...
    #     end
    #
    class Hybrid < Abstract

      class File

        def initialize(uploader, path = nil)
          @path = path
          @uploader = uploader
        end
        ...
        def url
          if @storage_mode.eql? :grid_fs
            if @uploader.grid_fs_access_url
        ...
        def storage_mode(mode)
          #defaults to grid_fs
          @storage_mode = :grid_fs
          @storage_mode = mode if mode.eql(:file)
        end
