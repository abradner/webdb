module NavigationHelpers
  ...
  def path_to(page_name)
    case page_name
      ...
      #Data Object paths
      when /the data object page for (.*)$/
        dobj = DataObject.first(:conditions => { :name => $1 })
        system_data_object_path(dobj.system_id, dobj.id)

      when /the add record page for (.*)$/
        dobj = DataObject.first(:conditions => { :name => $1 })
        new_system_data_object_data_object_row_path(dobj.system_id, dobj.id)
