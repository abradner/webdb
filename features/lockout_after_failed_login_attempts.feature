Feature: Locking out users after multiple failed password attempts
  ...
  Scenario: 3 consecutive failed logins results in account being locked.
    When I attempt to login with "super.user@intersect.org.au" and "blah"
  ...
  Scenario: A successful login after 2 failures resets the failure count to zero
    ...
    When I attempt to login with "super.user@intersect.org.au" and "blah"
    Then I should see "Invalid email or password."
    And the failed attempt count for "super.user@intersect.org.au" should be "2"
    When I attempt to login with "super.user@intersect.org.au" and "Pas$w0rd"
    Then I should see "Logged in successfully."
    And the failed attempt count for "super.user@intersect.org.au" should be "0"
    When I follow "Logout"
    And I attempt to login with "super.user@intersect.org.au" and "blah"
    Then I should see "Invalid email or password."
    And the failed attempt count for "super.user@intersect.org.au" should be "1"
